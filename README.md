...
1. Globals can be read and mutated by any part of the system, so makes it harder to understand and control
2. Concurrency issues - if globals can be accessed by multiple threads of execution, synchronization is necessary
3. harder to test
4. Static variables represent global state.  The tighter the scope of a variable, the easier to reason about
5. Statics are contrary to object-oriented paradigm (does not encapsulate data)
6. Static doe snot get garbage collected

* Tell me about Inversion of Control and how does it improve the design of code.
1. All about removing dependencies from your code
2. Giving control of dependencies to the client (when client creates uses your object, they can pass in the dependencies)
3. Separates what to do from when to do - and ensure the when knows as little as possible from the what part
 
Pros
  1. your code gets decoupled so you can easily exchange implmementations of an interface with alternative implementations
  2. strong motivator for coding against interfaces instead of implementations
  3. easy to wrtie tests, since no dependencies
...
1. Isolation Layer between a legacy system and new system.  Used to ensure that an application's design is not limited by dependencies on legacy systems
